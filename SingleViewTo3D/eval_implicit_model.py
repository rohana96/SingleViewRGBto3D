...
from utils import load_checkpoint, visualize_occupancy, sample_points_from_voxel, sample_voxel_grid_points_for_eval
import mcubes
import utils_vox
from visualizer.visualize_mesh import VisualizeMesh
from visualizer.visualize_point_cloud import VisualizePointCloud
...
def evaluate_model(args):
    ...
    model = ImplicitModel(n_points=(args.voxel_size**3))
    if torch.cuda.is_available():
        model.cuda()
    model.eval()
    ...
    if args.load_checkpoint:
        checkpoint_dir = f'{args.checkpoint_dir}{args.type}/resnet18_simple/checkpoint_{args.type}_4999.pth'
    ...
    for step in range(start_iter, max_iter):
        ...
        points_gt = sample_voxel_grid_points_for_eval(batch_size=args.batch_size, vox_size=32)
        read_time = time.time() - read_start_time

        predictions = model(points_gt, images_gt)
        predictions = predictions.view(args.batch_size, 1, 32, 32, 32)
